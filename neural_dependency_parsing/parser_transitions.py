...
def minibatch_parse(sentences, model, batch_size):
    ...
    partialParses = [PartialParse(sent) for sent in sentences]
    unfinisedParses = partialParses[:]
    while len(unfinisedParses) != 0:
        minibatchParses = unfinisedParses[:batch_size]
        transitions = model.predict(minibatchParses)
        for idx, trans in enumerate(transitions):
            minibatchParses[idx].parse_step(trans)
            if len(minibatchParses[idx].stack) == 1 and len(minibatchParses[idx].buffer) == 0:
                unfinisedParses.remove(minibatchParses[idx])
    dependencies = [parser.dependencies for parser in partialParses]
    ### END YOUR CODE
