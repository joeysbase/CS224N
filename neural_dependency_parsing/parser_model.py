...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(hidden_size, n_features * embeddings.shape[1])))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.zeros(hidden_size)))
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.zeros(n_classes, hidden_size)))
    ...
    def embedding_lookup(self, w):
        ...
        idxMat = w.flatten()
        x = self.embeddings.index_select(0, idxMat).view(w.shape[0], self.n_features * self.embeddings.shape[1])
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h = x.matmul(self.embed_to_hidden_weight.T) + self.embed_to_hidden_bias
        nonLinear = F.relu(h)
        logits = nonLinear.matmul(self.hidden_to_logits_weight.T) + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
