...
class NameDataset(Dataset):
    # checkout the dataset "birth_place_train.tsv" to gain some intuitions about why it is done so
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        doc = self.data[idx]
        truncated_size = random.randint(4, int(self.block_size * 7 / 8))
        if truncated_size > len(doc):
            masked_size = int(len(doc) / 4)
            # assert (len(doc) - masked_size - 1) > 1, f'{len(doc)}, {masked_size}, {idx}'
            split_start = random.randint(1, len(doc) - masked_size - 1)
        else:
            masked_size = int(truncated_size / 4)
            split_start = random.randint(1, truncated_size - masked_size - 1)
            doc = doc[:truncated_size]
        assert len(doc[split_start:split_start + masked_size]) != 0, f'split_start:{split_start},masked_size:{masked_size},len(doc):{len(doc)}, truncated_size:{truncated_size}'
        prefix, masked_content, suffix = doc.partition(doc[split_start:split_start + masked_size])
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (self.block_size - (len(doc) + 2 * len(self.MASK_CHAR)))
        inp = masked_string[:-1]
        oup = masked_string[1:]
        x = torch.tensor([self.stoi[s] for s in inp], dtype=torch.long)
        y = torch.tensor([self.stoi[s] for s in oup], dtype=torch.long)
        assert len(x) == self.block_size - 1
        return x, y
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        for i in range(len(corruption_dataset)):
            x, y = corruption_dataset[i]
