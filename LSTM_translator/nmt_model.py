...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        packed_enc_hiddens, (unreshaped_last_hidden, unreshaped_last_cell) = self.encoder(packed_X)
        enc_hiddens, lengths_unpacked = pad_packed_sequence(packed_enc_hiddens)
        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2))
        # print(unreshaped_last_hidden)
        reshaped_last_hidden = torch.cat(list(unreshaped_last_hidden), dim=1)
        # print(reshaped_last_hidden)
        reshaped_last_cell = torch.cat(list(unreshaped_last_cell), dim=1)
        dec_init_state = self.h_projection(reshaped_last_hidden), self.c_projection(reshaped_last_cell)
        ### END YOUR CODE
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for i in range(Y.shape[0]):
            Ybar_t = torch.cat([o_prev, Y[i]], dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = (dec_hidden * torch.permute(enc_hiddens_proj, (1, 0, 2))).sum(dim=2).T 
        ### END YOUR CODE
        ...
        a_t = (torch.unsqueeze(alpha_t, 2) * enc_hiddens).sum(dim=1)
